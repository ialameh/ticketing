...
var airport_input = function airport_input(id, data, options) {
  var listOfResults = [];
  var fuse = new _fuse.default(data, options.fuse_options);
  ...
  function search(e) {
    ...
    if (ac.value.length > 0) {
      ...
      var divs = results.map(function (r, i) {
        var result = options.formatting.replace(new RegExp('\\$\\(unique-result\\)', 'g'), autocomplete_result).replace(new RegExp('\\$\\(i\\)', 'g'), i).replace(new RegExp('\\$\\(name\\)', 'g'), r.name).replace(new RegExp('\\$\\(IATA\\)', 'g'), r.IATA).replace(new RegExp('\\$\\(city\\)', 'g'), r.city).replace(new RegExp('\\$\\(country\\)', 'g'), r.country);
        return result;
      });
